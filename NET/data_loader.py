...
class MnistLoader(object):
    DATA_SIZE = (1801, 1801)      # number of figures
    FIG_W = 15                      # width of each figure
    ...
    def _load(self, data_path='data'):
        ...
        self.data_train = np.load("/Users/chenyifan/Desktop/AfterImage/result.npy").astype(np.float32)
        self.label_train = np.load("/Users/chenyifan/Desktop/AfterImage/label.npy").astype(np.int64)
        self.data_test = np.load("/Users/chenyifan/Desktop/AfterImage/result.npy").astype(np.float32)
        self.label_test = np.load("/Users/chenyifan/Desktop/AfterImage/label.npy").astype(np.int64)
        # flatten data
        if self.flatten:
    ...
    def demo(self):
        ...
        # save the figure
        print(self.data_train[ind])
        plt.imshow(self.data_train[ind].reshape(self.FIG_W, self.FIG_W),cmap='Greys_r')
        plt.show()
        plt.savefig("/Users/chenyifan/Desktop/demo.png")
        # un.save()