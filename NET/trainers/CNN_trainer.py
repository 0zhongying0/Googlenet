...
def train(data, model, optimizer, logger, config):
    ...
    for epoch in range(config["last_epoch"] + 1, config["epochs"] + 1):
        ...
        for i in range(test_batches):
            ...
            inputs = Variable(torch.from_numpy(data.data_train[batch_indices, :]), requires_grad=False).view(-1, 1, 15, 15)
        ...
        for i in range(test_batches):
            inputs = Variable(torch.from_numpy(data.data_test[i * config["batch_size"]: min((i + 1) * config["batch_size"], data.DATA_SIZE[1]), :]), requires_grad=False).view(-1, 1, 15, 15)
            if torch.cuda.is_available():
                inputs = inputs.cuda()
            outputs = model(inputs)
